...
typedef struct 
{
	int head;
	int tail;
	topic_entry entries[maxentries];
}topic_queue;
...
typedef struct
{
	int head;
	int tail;
	topic_entry entries[num_publishers];
}wait_queue;



void enqueue(topic_queue which_q, topic_entry ent)
{
	if (isfull(which_q)) 
	{
	  printf("Overflow, new entry is waitting to inser.\n");
	}
	else 
	{
		which_q.entries[which_q.tail] = ent;
	}
}

int dequeue(topic_queue which_q)
{	
	int new_head =0;
	if(isempty(which_q))
	{
	...
	{
		new_head = which_q.head;
		which_q.head = (which_q.head +1) % maxentries;
...
int isfull(topic_queue which_q)
{
	return which_q.head == (which_q.tail + 1) % maxentries;
}

int isempty(topic_queue which_q)
{
	return which_q.head == which_q.tail;
}

int isnotempty(topic_queue which_q)
{
	return which_q.head != which_q.tail;
}

int isnotfull(topic_queue which_q)
{
	return which_q.head != (which_q.tail + 1) % maxentries;
}
...
void  *pub_connection(void* arg)
{
		...
		for(ap_index = 0; ap_index < (3); ap_index++)/// move to pub_connection
		{
			...
			if(ap_index == 0)
			{
				...
				while (split != NULL)
				{
					...
					if(split_index ==1)
					{	
						int split_pid = atoi(split);
						printf("Insert pubid %d\n", split_pid);
						ent.pubID = split_pid;
					}
				}		
			}
			...
			if(ap_index == 1)
			{	
				enqueue_topic = ap_buf[which_pub][16];
		...
		ent.timestamp = (int)time(NULL);
		enqueue(waitkun[enqueue_topic], ent);
		// if waitqueue is not empty and normal queue is not full
		if(isnotempty(waitkun[enqueue_topic]) && isnotfull(kun[enqueue_topic]))
		{
			enqueue(kun[enqueue_topic], waitkun[enqueue_topic].entries[waitkun[enqueue_topic].head]);
			dequeue(waitkun[enqueue_topic]);
		}
...
{	
		...
		if(isnotempty(waitkun[dequeue_topic]) && isnotfull(kun[dequeue_topic]))
			{
				enqueue(kun[dequeue_topic], waitkun[dequeue_topic].entries[waitkun[dequeue_topic].head]);
				dequeue(kun[dequeue_topic]);
			}
...
///////ATP Server	
	...
	{	
		...
		for (which_pub = 0; which_pub < num_publishers; which_pub++) 
		{
			...
			pthread_mutex_destroy(&pub_lock);
		}
